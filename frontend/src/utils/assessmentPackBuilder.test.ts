...
describe('assessmentPackBuilder', () => {
    ...
    it('autogenerated target IDs avoid collisions after deletions', () => {
        let domain: Domain = {
            domain_id: 'A',
            title: 'Domain A',
            targets: [
        ...
        domain = {
            ...domain,
            targets: domain.targets.filter((target) => target.target_id !== 'A1'),
        };
        domain = appendTargetToDomain(domain, [0, 1]);
        expect(domain.targets.map((target) => target.target_id)).toEqual(['A2', 'A1']);
    });

    it('does not invent a target ID when the domain ID is still empty', () => {
        const domain: Domain = { domain_id: '', title: '', targets: [] };
        expect(nextAutogeneratedTargetId(domain)).toBe('');
        expect(createBuilderTarget(domain, [0, 1, 2]).target_id).toBe('');
    });
});
